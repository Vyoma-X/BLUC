...
export default (io, socket) => {
  ...
  socket.on('disconnect-chat', handleDisconnectChat);
  socket.on('next', handleNext);
  socket.on('disconnect', () => cleanupUserConnections(socket.id));
  socket.on('video-offer', handleVideoOffer);
  socket.on('video-answer', handleVideoAnswer);
  socket.on('ice-candidate', handleIceCandidate);
  socket.on('end-call', handleEndCall);
  socket.on('reloading', handleReloading);

  function handleUserDetails({ gender, interest, name, mode, selectedGender }) {
    socket.data = { gender, interest, selectedGender };
    console.log(`User ${socket.id} joined with gender: ${gender}, interest: ${interest} for ${mode}`);
    cleanupUserConnections(socket.id);

    const waitingUsers = mode === 'video' ? videowaitingUsers : textwaitingUsers;
    let pastId = null, pastSocket = null;

    for (const [id, otherSocket] of waitingUsers) {
      if (id === socket.id) continue;

      const matchInterest = otherSocket.data?.interest === interest;
      const matchGender = otherSocket.data?.gender === socket.data.selectedGender;

      if (selectedGender === 'random' && matchInterest) {
        return connectUsers(id, otherSocket, mode);
      }

      if (matchInterest && selectedGender !== 'random') {
        if (matchGender) return connectUsers(id, otherSocket, mode);
        if (!pastId) [pastId, pastSocket] = [id, otherSocket];
      }

      if (!matchInterest && !pastId) [pastId, pastSocket] = [id, otherSocket];
    }

    if (pastId) return connectUsers(pastId, pastSocket, mode);

    waitingUsers.set(socket.id, socket);
    console.log(`User ${socket.id} added to ${mode} waiting list.`);
  }

  function handleSendMessage(message, toSocketId) {
    const target = io.sockets.sockets.get(toSocketId);
    if (target) target.emit('receive-message', message);
  }

  function handleDisconnectChat(partnerSocketId, mode) {
    const partnerSocket = io.sockets.sockets.get(partnerSocketId);
  ...
  function handleNext(partnerSocketId, mode) {
    const partnerSocket = io.sockets.sockets.get(partnerSocketId);
    if (mode === 'video') handleVideoCallEnd(socket.id, partnerSocketId);
    if (partnerSocket) partnerSocket.emit('find other');
    socket.emit('find other');
  }

  function handleVideoOffer(offer, toSocketId) {
    const target = io.sockets.sockets.get(toSocketId);
    if (target) {
      target.emit('video-offer', offer, socket.id);
      activeVideoCalls.add(`${socket.id}-${toSocketId}`);
  ...
  function handleVideoAnswer(answer, toSocketId) {
    const target = io.sockets.sockets.get(toSocketId);
    if (target) target.emit('video-answer', answer);
  }

  function handleIceCandidate(candidate, toSocketId) {
    const target = io.sockets.sockets.get(toSocketId);
    if (target) target.emit('ice-candidate', candidate);
  }

  function handleEndCall(partnerId) {
    videowaitingUsers.delete(socket.id);
    const partnerSocket = io.sockets.sockets.get(partnerId);
    if (partnerSocket) {
      partnerSocket.emit('end-video');
  ...
  function connectUsers(id, otherSocket, mode) {
    const waitingUsers = mode === 'video' ? videowaitingUsers : textwaitingUsers;
    waitingUsers.delete(id);

    const matchedSocket = io.sockets.sockets.get(id);
    if (!matchedSocket) return;
  ...
  function cleanupUserConnections(userId) {
    ...
    activeVideoCalls.forEach(callId => {
      if (callId.includes(userId)) activeVideoCalls.delete(callId);
    });
  ...
  function handleReloading() {
    ...
    if (partnerId) {
      ...
      if (partnerSocket) {
        partnerSocket.emit('find other');
        partnerSocket.emit('disconect', 'Partner reloaded.');
      }
      handleVideoCallEnd(socket.id, partnerId);
    }
    cleanupUserConnections(socket.id);
